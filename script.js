document.getElementById('calculateBtn').addEventListener('click', function() {
    ...
    if (ei && es && x && sigma) {
        ...
        const zr1 = (F(z1)*100).toFixed(2); // Годные детали
        const zr2 = (F(z2)*100).toFixed(2); // Бракованные детали
        const zr3 = (F(z3)*100).toFixed(2); // Неисправимый брак
        const zr4 = (F(z4)*100).toFixed(2);
        const zr5 = ((F(z1) - F(z2)) * 100).toFixed(2); // Процент годных к бракованным
        const zr6 = ((F(z2) - F(z3)) * 100).toFixed(2); // Процент бракованных к неисправимым
        const zr7 = ((F(z4) - F(z1)) * 100).toFixed(2); // Процент исправимых к годным
    
        document.getElementById('result1').innerHTML = `Количество годных деталей: ${zr1}% - ${zr2}% = <strong>${zr5}</strong> %`;
        document.getElementById('result2').innerHTML = `Количество неисправимого брака: ${zr2}% - ${zr3}% = <strong>${zr6}</strong> %`;
        document.getElementById('result3').innerHTML = `Количество исправимого брака: ${zr4}% - ${zr1}% = <strong>${zr7}</strong> %`;
    } else {
